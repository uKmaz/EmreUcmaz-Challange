...
public fun battle(hero: Hero, arena: Arena, ctx: &mut TxContext) {
    ...
    let hero_id_copy = object::id(&hero);
    let warrior_id_copy = object::id(&warrior);
    ...
    // TODO: Compare hero.hero_power() with warrior.hero_power()
        ...
        if (hero.hero_power() > warrior.hero_power()) {
            ...
            event::emit(ArenaCompleted {
                winner_hero_id: hero_id_copy,
                loser_hero_id: warrior_id_copy,
                timestamp: ctx.epoch_timestamp_ms()
            });
            ...
            event::emit(ArenaCompleted {
            winner_hero_id: warrior_id_copy,
            loser_hero_id: hero_id_copy,
            timestamp: ctx.epoch_timestamp_ms()
            });
        };
